...
window.onload = function() {
  ...
  document.querySelectorAll('.room-card').forEach((room_card) => {
    ...
    if (count == room_capacity) {
      filled_count += 1;
      // console.log(filled_count);
      room_card.classList.add('filled');
    } else if (0 < count && count < room_capacity) {
      room_card.classList.add("partial");
      partial_count += 1;
    } else if (count == 0) {
      // عيملين حلقة بتلف عكل الغرف للمبنى بالتالي  في كل لفة رح تزيد لحتى توصل للعدد الكلي
      // عند وجود طالب في غرفه فلن يتحقق الشرط الاخير وبالتالي ستنقص غرفة
      vacant_count += 1;
    }

    // هذا يستعرض عند التمرير فوق الغرفه رقم الطالب المسجل
    $(room_card).hover(() => {
...
function placeDetails(room_str, floor, no, room_students) {
  ...
  room_students.forEach(room => {
    ...
    row.innerHTML = `
      ...
      // في قاموس الطلب يلتقط النييم تبع الحقل والقيمة 
      // <QueryDict: {'roomdetail_id': ['6'], 'remove': ['Remove']}>
      // بينما اذا اردنا التقاط الاي دي يجب وضع الايدي في النييم كما التعليمه في الاسفل
      // <input type='submit' id="remove_id" name='id' value='Remove' class='btn btn-danger' />\
